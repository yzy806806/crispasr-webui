...
# ─── Write password file ──────────────────────────────────
ENV_FILE="/etc/crispasr-webui.env"
if [ "$OS" = "linux" ]; then
    cat > "$ENV_FILE" << EOF
...
ExecStart=${BINARY_DIR}/crispasr --server --backend ${BACKEND} -m ${MODEL_FLAG} --voice-dir ${INSTALL_DIR}/voices --host 127.0.0.1 --port ${CRISPASR_PORT} -t ${THREADS} ${GPU_FLAGS}
Restart=on-failure
RestartSec=10
